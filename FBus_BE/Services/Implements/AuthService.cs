                    ...
                    switch (account.Status)
                    {
                        case "UNSIGNED":
                            ...
                            if (account.Password == payload.Subject)
                            {
                                ...
                                JwtSecurityToken jwtSecurityToken = new JwtSecurityToken(issuer, audience, new List<Claim> { new Claim("Id", account.Id.ToString()), new Claim("Role", account.Role) }, null, DateTime.Now.AddMinutes(30), signingCredentials);
